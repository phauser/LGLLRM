...
inline double postDelta(std::size_t i, 
  ...
  double lp = arma::as_scalar(-0.5*tau*pow(delta(i),2) + 
                ...
                sum(sum(sum(log(1+exp(X*U*diagmat(delta)*V.t()+Z*b+eta*lambda.t()))))));
  return lp;  
}
...
inline double postLambda(std::size_t i, 
  ...
  double lp = arma::as_scalar(-0.5*lambda.row(i)*Dinvi*lambda.row(i).t() +
             ...
             sum(sum(log(1+exp(X*U*arma::diagmat(delta)*V.row(i).t()+Z*b.col(i)+eta*lambda.row(i).t())))));
  return lp;  
}
...
inline double postEta(std::size_t i, 
    ...
    const arma::mat &b,  const arma::mat &eta, const arma::mat &lambda, 
    const arma::mat &U,  const arma::mat &V,   const arma::vec &delta,  double tau) {
  // figure out which row of Y, X, and Z corresponding to the jth row within subject i's mi's rows
  // int n  = m.n_elem;
  // int pb = b.n_rows/n;
  // double ind=0;
  // if(j>m[i-1]){throw std::range_error("Inadmissible value");}
  // else{
  //  if(i==1){ind=j-1;}
  //  else{ind = sum(m.subvec(0,i-2))+j-1;}
  // }
  
  // double lp = arma::as_scalar(-0.5*tau*eta.row(ind)*eta.row(ind).t()
  //              + Y.row(ind)*lambda*eta.row(ind).t()
  //              - sum(log(1+exp(X.row(ind)*U*diagmat(delta)*V.t()+Z.row(ind)*b.row(ind*pb)+eta.row(ind)*lambda.t()))));
  
   double lp = arma::as_scalar(-0.5*tau*eta.row(i)*eta.row(i).t()
                + Y.row(i)*lambda*eta.row(i).t()
                - sum(log(1+exp(X.row(i)*U*diagmat(delta)*V.t()+Z.row(i)*b+eta.row(i)*lambda.t()))));
  return lp;  
}
...
double postb(std::size_t i, std::size_t k, 
  ...
  double lp = arma::as_scalar(-0.5*tau*b_ik.t()*b_ik + Y_ik.t()*Z_i*b_ik - 
                               sum(log(1+exp(X_i*U*diagmat(delta)*V.row(k).t()+Z_i*b_ik+eta_i*lambda.row(k).t()))));
  return lp;  
}
      ...
      for(std::size_t j = 0; j < b.ncol(); j++)
      {
        ...
        if(proposal=="Gamerman"){
          ...
          for(int j1=0; j1<d; j1++){
            ...
            W_ij0(j1) = arma::as_scalar(exp(theta_ij)/pow(1+exp(theta_ij),2));
            Y_XB0(j1) = eta_ij+pow(1.0+exp(theta_ij),2)/exp(theta_ij)*(y(i,j1)-exp(theta_ij)/(1.0+exp(theta_ij)));
          }
...
// [[Rcpp::export]]
List LGLLRM(const arma::mat &Y, const arma::mat &X, const arma::mat &Z, 
    const arma::vec & m, int pb, 
    double & epsEta, double & epsb, double & epsU, double & epsV, double & epsD, double & epsL,
    int & r, int & q, int & nBurnin, int & nCollect, int & thin, int numThreads){
  ...
  mat eta        = zeros(N, q);
  double alpha1  = 0.01;
  double tauEta   = rgamma(1, alpha1, 1.0/alpha1)(0); // Rcpp is shape and scale, R is shape and rate
  ...
  for(int iter=0; iter<runs; iter++){
    ...
    tauEta = rgamma(1, 0.5*(N*q+ alpha1), 1.0/rate(0))(0); 
    varEta = 1.0/tauEta;
    ...
    arma::mat ETA = X*B+Z*b+eta*lambda.t();
    arma::mat ll10 = Y.t()*ETA;
    double ll20 = arma::as_scalar(sum(sum(log(1+exp(ETA)))));
...
List PerformGLMM(mat Y, mat X,  mat Z, mat B, mat E, mat b, int r, vec dev0){
  mat ll1 = Y.t()*(X*B+E);
  double ll2 = arma::as_scalar(sum(sum(log(1+exp(X*B+E)))));
...
// [[Rcpp::export]]
List SigGLMM(cube B, double d0, double p0, std::string dir){
  int p = B.n_rows;
  int d = B.n_cols;
  mat prob = zeros(p,d);
  mat B1 = zeros(p,d);
  ...
  for(int i=0; i<p; i++){
    for(int j=0; j<d; j++){
      vec b = B.tube(i,j);
      if(dir=="l"){prob(i,j) = sum(exp(b)<d0)/slices;}
      if(dir=="g"){prob(i,j) = sum(exp(b)>d0)/slices;}
